...
**The Solution:** Apply a "Fail Up" strategy to retrieval logic, similar to the extraction tiers.
...
* **Example:** "Who is the *manager* of the project that generated the *highest cost* invoice in *Q3*?" (Connects `Org Chart` $\rightarrow$ `Project Codes` $\rightarrow$ `Invoice Data`).
